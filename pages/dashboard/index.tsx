        ...
        <>
          <div className="mb-4 flex items-center gap-2">
            <button
              ...
              disabled={busy || items.length > 0 || needsApproval}
              title={
                items.length > 0
          ...
          <ul className="space-y-3">
            {items.map((b) => (
              <li key={b.id} className="border rounded p-3 flex items-center justify-between">
                ...
                <Link href={`/dashboard/${b.id}`} className="px-3 py-1 bg-blue-600 text-white rounded">
                  Editar
                </Link>
              </li>
            ))}
            {items.length === 0 && (
              <p className="text-gray-500">Aún no tienes negocios.</p>
            )}
          </ul>
        </>
      )}
