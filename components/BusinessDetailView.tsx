...
const _ImageGallery = dynamic(() => import("react-image-gallery"), {
...
export default function BusinessDetailView({ business, onGalleryStateChange }: Props) {
  ...
  const theme = {
    ...
    wrapper: 'bg-white border-t-4 border-purple-500',
      ...
      heroHeight: getHeroHeight('sponsor'),
    ...
    featured: {
      ...
      heroHeight: getHeroHeight('featured'),
  ...
  return (
    <>
      ...
      <div className="px-4 py-6 space-y-6">
        ...
        <div className="px-4 py-5 relative z-10">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 mt-5 border-t border-gray-100 pt-4 -mx-4 px-4 scrollbar-hide">
            ...
            {showOrderCart && (
              <button
                onClick={() => document.getElementById('pedido-section')?.scrollIntoView({ behavior: 'smooth' })}
                className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm font-medium transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2m0 0L7 13h10l2-8H5.4m0 0L4.2 9M7 16a1 1 0 100 2 1 1 0 000-2zm10 0a1 1 0 100 2 1 1 0 000-2z" />
                </svg>
                Pedidos
              </button>
            )}
      ...
      {/* Galería de Fotos - Condicional por plan */}
      <section id="galeria-section" className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
        ...
        {plan === 'free' && detailTokens.showGalleryBlock ? (
          <div className="relative rounded-xl border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-50 to-gray-100 p-8 text-center overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                {detailTokens.galleryBlockTitle}
              </h3>
              <p className="text-sm text-gray-600 mb-6 max-w-md mx-auto">
                {detailTokens.galleryBlockMessage}
              </p>
              
              {/* CTA */}
              <Link
                href={detailTokens.galleryBlockCTAHref || '/para-negocios'}
                className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-0.5"
                onClick={() => trackDetailInteraction('gallery_block_cta_clicked')}
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          (plan === 'featured' || plan === 'sponsor') && allGalleryImages.length > 0 && !dataSaverEnabled ? (
            ...
            /* Sin fotos: Mensaje diferenciado por plan */
...
const _PhoneIcon = ({ className = "w-4 h-4", ...props }: IconProps) => (
  <svg
    viewBox="0 0 20 20"
    fill="currentColor"
    role="img"
...
const _WhatsappIcon = ({ className = "w-4 h-4", ...props }: IconProps) => (
  <svg
    viewBox="0 0 20 20"
    fill="currentColor"
    role="img"
...
const _LocationIcon = ({ className = "w-4 h-4", ...props }: IconProps) => (
  <svg
    viewBox="0 0 20 20"
    fill="currentColor"
    role="img"
